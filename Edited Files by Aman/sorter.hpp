...
struct Client
{
  Client(const std::string& _name, double _share, uint64_t _allocations, float _CPUS_PER_TASK, float _MEM_PER_TASK, float _GPUS=0.0, float _DISK=0.0)
    : name(_name), share(_share), allocations(_allocations), CPUS_PER_TASK(_CPUS_PER_TASK), MEM_PER_TASK(_MEM_PER_TASK), GPUS(_GPUS), DISK(_DISK){}
...
public:
  ...
  virtual void add(const std::string& name, double weight = 1, const FrameworkInfo& frameworkInfo);
