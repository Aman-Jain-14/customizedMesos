...
void DRFSorter::add(const string& name, double weight, const FrameworkInfo& frameworkInfo)
{
  float CPUs, MEM, GPUS = 0.0, DISK 0.0;
  CPUs = frameworkInfo.cpus_per_task();
  MEM = frameworkInfo.mem_per_task();
  if(frameworkInfo.gpus())
    GPUS = frameworkInfo.gpus();
  if(frameworkInfo.disk())
    DISK = frameworkInfo.disk();


  CHECK(!contains(name));

  Client client(name, 0, 0, CPUs, MEM, GPUS, DISK);
